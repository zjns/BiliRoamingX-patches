    ...
    override fun execute(context: BytecodeContext): PatchResult {
        ...
        SpeedFunctionWidgetFingerprint.result.mapNotNull { r ->
            r.classDef.fields.firstNotNullOfOrNull { f ->
                context.classes.first { it.type == f.type }.takeIf {
            ...
            m.addInstructions(
                insertIndex, """
                invoke-static {p1}, Lapp/revanced/bilibili/patches/PlaybackSpeedPatch;->getOverrideSpeedArray([F)[F
        ...
        MenuFuncSegmentFingerprint.result?.mutableClass?.fields?.find { it.type == "[F" }?.let { f ->
        ...
        context.findClass("Lcom/bilibili/music/podcast/view/PodcastSpeedSeekBar;")
            ?.mutableClass?.run {
        ...
        PlayerSpeedWidgetFingerprint.result.mapNotNull { r ->
            r.mutableClass.methods.find {
                it.implementation?.registerCount == 8 && it.returnType == "V" && it.parameterTypes.isEmpty()
            }
        }.ifEmpty {
            return PlayerSpeedWidgetFingerprint.toErrorResult()
        }.forEach { m ->
            val insertIndex = m.implementation!!.instructions.indexOfFirst {
                it.opcode == Opcode.CONST && it is BuilderInstruction31i && it.wideLiteral == 0x3ffeb852.toLong()
            }
            val oneIndex = m.implementation!!.instructions.indexOfFirst {
                it.opcode == Opcode.CONST_HIGH16 && it is BuilderInstruction21ih && it.wideLiteral == 0x3f800000.toLong()
            }
